...
import requests
import uuid
import pandas as pd   #for reading and storing documents 
...
from werkzeug.utils import secure_filename #for safer side if any syntax is wrong for filename
...
# Normalize to "1"/"2"
    ...
    try:
        ...
        answer = None
        if hasattr(choice, "message"):  #Extracts the actual text answer from AI.
            msg = choice.message
    ...
    # fallback
    if mode == "1":
        answer = "(From local source)\n- Demo mode active (AI unavailable)\n- Showing system functionality\n- Documents are processed correctly"
    else:
        answer = "(From Global source)\n- Demo mode active (AI unavailable)\n- Backend and chat flow are working\n- API can be re-enabled with credits"
...
def switch_mode():
    ...
    try:
        query_id = ObjectId(session_id)  #Convert the session_id string into an ObjectId as Some sessions are stored ObjectId.
